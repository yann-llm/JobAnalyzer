...
def build_company_risk_messages(
    ...
    *,
    ...
    user_static = (
        ...
        "优先从 `company.registration_info`、`company.controller`、`company.shareholdinfo`、`company.investments` 提取。"
        ...
        "`一句话总结`。"
        "评分权重：工商与存续状态25%，股东/控制人稳定性20%，经营规模与参保人数20%，"
        "企查查风险记录25%，对外投资与主体扩张稳定性10%。"
        "硬性规则："
        ...
        "`组织规模可信度`(`高`/`中`/`低`/`数据不足`)、"
        "`资本结构稳定性`(`高`/`中`/`低`/`数据不足`)、"
        "`扩张稳定性`(`高`/`中`/`低`/`数据不足`，结合对外投资数量、状态和时间判断)。"
