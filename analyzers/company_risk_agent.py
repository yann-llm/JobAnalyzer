...
def build_company_risk_messages(
    ...
    *,
    ...
    user_static = (
        ...
        "`text`(1-3句分析正文，必须基于 `company` 输入证据)、"
        ...
        "1. `kpis` 必须正好 4 项；"
        "2. `label` ≤ 8 字，简短指标名；"
        "3. `val` 必须是「关键值」：可量化数字、级别词或具体标签词，如 `存续`、`0/4`、`不活跃`、`国家高新`、`2 项`；"
        "禁止用「未提供」「数据不足」单独作为 val（应在 sub 里说明），除非企查查明确显示数据不足；"
        "4. `sub` 必须是补充说明，按以下优先级填："
        "  (a) 客观引用：引用企查查字段原文（例：`登记状态：存续（在营、开业、在册）`）；"
        "  (b) 列举：列出关键标签或具体记录（例：`命中风险：失信 1 项；其余 3 项 0 命中`）；"
        "  (c) 定量：数字 + 上下文（例：`成立 6 年、注册资本 100 万`）；"
        "  (d) 没有任何具体数据时，老实写 `企查查无记录` 或 `输入未提供`；"
        "禁止编造（不得出现：脉脉、看准网、Glassdoor、市场分位 P82 等具体数字）；"
        "5. `source` 只能从白名单选：`企查查` / `JD原文` / `LLM推断`；"
        "工商/股东/风险/招聘/荣誉等事实选 `企查查`，定性判断选 `LLM推断`。"

        "推荐 `统一评分.kpis` 4 项（val 是关键值，sub 是引用或列举）："
        "`{label:'存续状态', val:'存续' 或 '吊销' 等关键词, sub:'引用 registration_info.登记状态 原文', source:'企查查'}`、"
        "`{label:'风险信号', val:'N/4'（4 项风险中命中 N 项）, sub:'命中：失信1项；其余 3 项 0 命中'/'4 项风险全部 0 命中', source:'企查查'}`、"
        "`{label:'招聘活跃度', val:'活跃/一般/不活跃/数据不足', sub:'引用 operation.recruitment 摘要例 \"暂无招聘记录\"', source:'企查查'}`、"
        "`{label:'荣誉资质', val:'2 项' 数量 或 '0' 或 '国家高新+专精特新' 标签, sub:'列出关键资质或填 企查查无记录', source:'企查查'}`。"
