...
def build_final_evaluation_messages(
    ...
    system_prompt = (
        "你是专业的招聘顾问助手。"
        "你会收到针对同一个职位的多个子分析结果（基础信息、岗位职责、候选人要求、薪酬与福利、"
        "公司与团队、工作强度、法律合规风险、行业与赛道前景、公司财务健康度），"
        "可能还会附带 `candidate_profile`（候选人画像）。"
        "请整合所有信息，给出最终的岗位画像和申请建议。"
        "不得编造子分析中未出现的数据。若 `industry_outlook` / `company_finance` 中"
    ...
    user_static = (
        ...
        "评分锚点（共100%）："
        "薪酬与福利20%，公司与团队10%，公司财务健康度10%，岗位职责清晰度10%，"
        "候选人要求合理性10%，工作强度合理性10%，法律合规风险10%，"
        "行业与赛道前景15%，基础信息完整度5%。"
        "硬性下调规则（任意一条命中即生效）："
        "(a) `legal_risk.整体风险等级.等级` 为 `高风险，谨慎` → 综合评分不得超过 49；"
        "(b) `legal_risk.整体风险等级.等级` 为 `需重点核实` → 综合评分不得超过 69；"
        "(c) `work_intensity.强度评级.等级` 为 `极高强度` → 综合评分不得超过 64；"
        "(d) `company.潜在风险` 任一桶累计达到 2 条以上 → 综合评分不得超过 79；"
        "(e) `company_finance.总体评级` 为 `D` → 综合评分不得超过 49；"
        "(f) `industry_outlook.短期前景_3年.方向判断` 为 `明显走弱` → 综合评分不得超过 64。"
        ...
        "`关键词`(字符串数组，最多8个)、`一句话总结`三个键；关键词需保留原文出现的技术名词或业务名词。"
        "`匹配建议`为对象，包含以下键："
        ...
        "优先级：法律/财务 > 强度 > 行业 > 业务/管理。"

        "—— 人群与前景 ——"
        "`适合人群`为字符串数组，最多4条，给出适合投递该岗位的候选人类型（如`3-5年全栈工程师`、`想转 AI 工程的后端`）。"
        "`短期前景_3年`为对象，包含三个键："
        "`业务稳定性`(`高`/`中`/`低`/`数据不足`)、`核心风险`(字符串，一句话)、`机会`(字符串，一句话)。"
        "需综合 `company_finance.财务健康度`、`industry_outlook.短期前景_3年`、`work_intensity` 给出判断。"
        "`长期前景_5_10年`为对象，包含三个键："
        "`成长天花板`(`高`/`中`/`低`/`数据不足`)、`赛道判断`(字符串，一句话谈行业 5-10 年的位置)、"
        "`转换能力`(字符串，一句话，谈这段经历能转向哪些方向)。"
        "需综合 `industry_outlook.长期前景_5_10年`、`company_finance.业务护城河`、`responsibilities.技术栈` 等给出判断。"
        "若 `industry_outlook` / `company_finance` 中相关项标注 `信息来源` 为 `模型常识`，须在 `赛道判断` 末尾加 `(基于行业常识)`。"
        ...
        "`必问问题`(字符串数组，最多4条，从 `legal_risk.建议追问的问题`、`work_intensity.数据缺口`、"
        "`company_finance.数据可信度.建议用户补充`、`industry_outlook.数据可信度.建议用户补充` 中挑出最关键的)、"
        ...
        "{若有候选人画像：**vs 候选人**：{匹配等级}}\n\n"
        "- 💰 **薪酬**：{薪资范围} · {薪酬竞争力评估等级}\n"
        "- 🏭 **公司**：{公司名称} · {规模} · {行业} · {company_finance 总体评级}\n"
        "- 🚀 **岗位**：{岗位定位} · {核心任务一句话}\n"
        "- 📊 **工作强度**：{work_intensity 星级} {等级} · {压力源最关键一条}\n"
        "- ⚠️ **法律风险**：{legal_risk 星级} {等级} · {最关键一条关注点}\n"
        "- 🌐 **行业**：{阶段} · {短期方向判断} · {长期方向判断}\n"
        "- ✅ **亮点**：{优势亮点最多3条，用 / 分隔}\n"
